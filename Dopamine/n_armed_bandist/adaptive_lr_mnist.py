...
class NN(object):
	...
	def __init__(self, x, y, action_dim, in_shape=(28, 28, 1), num_classes=10):
		'''
		constructor
		...
		(self.base_model).add(Conv2D(32, kernel_size=(3, 3), activation="relu", input_shape=in_shape))
		(self.base_model).add(MaxPooling2D(pool_size=(2, 2)))
		
		self.main_model = Sequential()
		(self.main_model).add(self.base_model)
		(self.main_model).add(Conv2D(64, kernel_size=(3, 3), activation="relu"))
		(self.main_model).add(MaxPooling2D(pool_size=(2, 2)))
		(self.main_model).add(Flatten())
		(self.main_model).add(Dropout(0.5))
	...
	def train(self, num_episodes, lrs, exploration_rate=0.1) -> None:
		...
		for i in range(num_episodes + 1):
			with tf.GradientTape() as tape:
				...
				x_test, y_test = self.get_sample(n_batch = 100)
...
learning_rates = np.random.rand(100) #np.array([0.9, 0.1, 0.01, 0.09])
