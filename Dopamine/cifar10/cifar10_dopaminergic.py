...
h5 = Flatten()(h4)
h6 = Dropout(0.5)(h5)
out = DopDense("softmax", num_classes, 5, 0.00018, 2)(h6)
dop_model = Model(inputs=inp, outputs=out)
dop_model.compile(loss='categorical_crossentropy', optimizer=opt, metrics=['accuracy'])

dop_model.summary()

dop_history = dop_model.fit(x_train, y_train, epochs=150, batch_size=64, verbose=1, callbacks=[MyCallback(epoch_variable)], validation_split=0.2)
