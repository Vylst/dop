		...
		if(self.train_test()):
			...
			if(self.batch_ctr > 1):
				...
				#for i in range(self.units):
					...
					#if(i not in self.dop_indices):
						#Create eta for this non-dopaminergic neuron
				...
				#	if( dop_neuron_mean_difference[self.dop_indices[i]] > self.threshold and self.batch_ctr-self.indicator[i] > self.ref_period):
						...
						#COMPUTE TRACE
						#trace = ((self.gamma*dop_neuron_mean_difference[self.dop_indices[i]]) * aux_factor ) + (1-self.gamma)*trace
				...
				#EACH CONVENTIONAL NEURON WILL NOW NEED ITS OWN FILTER TO ACCOUNT FOR THE ETA FACTOR DETERMINING THE WEIGHT OF EACH
				#DOPAMINERGIC NEURON'S TRACE OVER IT (I.E. ITS OUTPUT)
...
dop_history = dop_model.fit(x_train, y_train, epochs=100, batch_size=64, verbose=1, callbacks=[MyCallback(epoch_variable)], validation_split=0.2)
...
with open("dop_history_" + str(n_dop_neurons) + "_dop_neurons.pkl", 'wb') as fp:
